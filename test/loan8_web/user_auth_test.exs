defmodule Loan8Web.UserAuthTest do
  use Loan8Web.ConnCase, async: true

  alias Phoenix.LiveView
  alias Loan8.Accounts
  alias Loan8.Accounts.Scope
  alias Loan8Web.UserAuth

  import Loan8.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, Loan8Web.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      Loan8Web.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        Loan8Web.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
